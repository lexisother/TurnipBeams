 ...
 */
export function hasPermission(user: User, member: Member | null, permission: number): boolean {
 ...
 */
export function getPermissionLevel(user: User, member: Member | null): number {
  for (let i = permissionLevels.length - 1; i >= 0; i--)
    if (permissionLevels[i].check(user, member)) return i;
  return 0;
}
