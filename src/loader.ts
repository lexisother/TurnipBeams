...
export async function loadCommands(
...
): Promise<Map<string, NamedCommand>> {
  ...
  const commands = new Map<string, NamedCommand>();
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      if (command instanceof NamedCommand) {
        ...
        console.warn(
          `Command "${commandID}" has no default export which is a NamedCommand instance!`,
        );
      }
    } catch (error) {
      console.error(error);
 ...
 */
export async function getCommandList(): Promise<Map<string, NamedCommand[]>> {
  const list = new Map<string, NamedCommand[]>();
...
export async function getCommandInfo(args: string[]): Promise<[CommandInfo, string] | string> {
  ...
  if (!(command instanceof NamedCommand))
    return 'Command is not a proper instance of NamedCommand.';
